...
| Piston | `blocks/piston`, `entities/piston.rs` | `PistonBaseBlock`, `PistonMovingBlockEntity` | mostly; same-tick retraction, start-of-tick honey rider movement, vanilla `is_pushable` push rules (obsidian family, bedrock, extended/unextended pistons, containers/block entities) and quasi-connectivity power checks are aligned; full event ordering fixtures remain |
...
| Crafter | `block/entities/crafter.rs`, `block/blocks/redstone/crafter.rs` | `CrafterBlock`, `CrafterBlockEntity` | mostly; shaped/shapeless matching, disabled slots, Java NBT, visible six-tick `CRAFTING` lifecycle, comparator refresh, component-preserving output, atomic snapshot-validated input consumption, vanilla result-then-remainder emission, 17-block radius `crafter_crafted_crafter` advancement dispatch, and sided hopper insertion/lower-count slot ordering work; world boundary fixtures and special recipe remaining-item edge cases remain |
...
| Minecarts | `entity/vehicle/minecart`, `entity/vehicle/vehicle.rs` | `AbstractMinecart`, `MinecartBehavior`, `MinecartCommandBlock`, `MinecartSpawner`, `VehicleEntity#hurtServer` | mostly; all seven subtypes have distinct state, command carts execute every four powered activator ticks and accept the Java editor packet, spawner carts reuse the full weighted/NBT spawner state, rail/projectile movement keeps position, block position and chunk tracking synchronized, and damage vibrations cover ordinary/TNT prime paths; Bedrock editor/UI, entity events and collision edge cases remain |
