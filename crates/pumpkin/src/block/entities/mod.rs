...
pub trait BlockEntity: Any + Send + Sync {
    ...
    /// Restores a deferred loot-table key that was taken for generation but
    /// could not be resolved or evaluated.  Keeping this operation on the
    /// block-entity contract makes unknown/future datapack tables lossless:
    /// opening a chest must never silently delete its `LootTable` NBT.
    fn restore_loot_table(&self, _key: String, _seed: i64) {}
